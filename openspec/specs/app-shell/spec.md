...
⚠ mark — red for the missing Accessibility permission (critical; upgraded from the
prior monochrome glyph), orange for gesture conflicts; the permission mark wins when
both apply. The menu-bar status glyph keeps showing ⚠ for either cause. EVERY
conflict-driven mark (menu item, status glyph, Settings tab — see the settings spec)
SHALL appear only while gestures are enabled: with gestures off a conflict is not
critical and MUST NOT distract. Marks refresh live on settings changes and on menu
open (existing refresh points).
